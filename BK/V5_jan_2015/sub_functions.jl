...
function fnc_optimal_PCrp()
    ...
    if log(b/a)>.5
        tau1 = max(1.01,(a/v * sqrt( log(b/a) - (1./2.)))) .* 1.5
    else
...
function fnc_taus1()
    ...
    #a = PC_f+PF_sig#min(PF_sig,PF_n*PC_f/(2*pi) )
    b = (a+GRD_mx/sqrt(PS_n) )/2.
    t1 = (1./(1.-PC_rp)) #avg time of straight flight
    t2 = 1./(PC_rp) #avg time of tumble search
    xy = sqrt(2)/v/t2
    x = a*xy
    y = b*xy
    ...
    return (t1+t2)/(2*t1*b^2) * result3 .* 1.5
end
...
function fnc_tau(dTs,dHs,cons,EVENTS,FLAGS,turns)
    ...
    for I=EVENTS["found_school"]
        ...
        @inbounds Ts[I] = Ts_old + ((ts[I]-Ts_old)/ns[I]) # run mean
        @inbounds Tv[I] = Tv_old + ((ts[I]-Ts_old)*(ts[I]-Ts[I])) # run variance
        
        @inbounds dTs[I] = abs(Ts[I]-Ts_old)/Ts[I]; # fractional change in mean
        @inbounds ts[I] = 1; # reset how long it took to find school
        if FLAGS["measure_H"]
            if cons.H[I]>1
                cons.measure["Hrate"][I]=cons.H[I]/turns
                @inbounds dHs[I]=abs(1+ (dHs[I]-10.)/cons.H[I]*turns) 
...
function fnc_fishfinder(school,fish,cons,fishtree,EVENTS,FLAGS)
    ...
    for i = 1:PC_n
        ...
        if FLAGS["implicit_fish"]
            ...
            for j=1:PS_n
                @inbounds D=fnc_dist(Cx[i,:],Sx[j,:])
                if D.<((2.5.*PF_sig).+PC_f)
                   @inbounds II[i]=[II[i],j]
                end
            end
        end
    end
...
function fnc_steam(school,fish,cons,fishtree,EVENTS,FLAGS)
    ...
    for i =  1:PC_n
    	...
    	if MI[i] == 1 # if ballistic
            if rand() < 1-PC_rp # probability of search-to-ballistic switch
				@inbounds MI[i] = 0
				#V[i]  = PC_v
            end
        elseif MI[i] == 0 # if searching
            if rand() < PC_rp # prob of ballistic-to-search switch
                @inbounds MI[i] = 1
                #V[i] = PC_v/3.
    ...
    for id = 1:PC_n
        ...
        if in(id,EVENTS["in_contact"]) #If you have just talked with someone
            ...
            for i = 1:Jn # for each friend
                @inbounds ii = J[i]; # get his/her index
        ...
        if  !isnan(Dmin)  && (isnan(current) || Dmin<current ) 
            ...
            cons.target[id]=jj
            cons.V[id]=PC_v # reset speed if so
        elseif  in(id,EVENTS["targeting"]) && !isnan(current) 
            ...
            if MI[id] == 0 # # steam fast
				Dxy = dxy[id,:]; # straight line
				V[id] = PC_v
            else# search/tumble slowly
