...
function do_timingtest(fsave=true)
    ...
    FLAGS["save"]=fsave
    FLAGS["ensemble"]=1
    @time  make_ensemble(school,fish,cons,fishtree,EVENTS,FLAGS,1,OUT);
end
...
function do_1fisher_1school_a()
    ...
    for i = 1:length(RP) # Iterate over different turning probs
    	for j = 1:NN # run sim a number of times 
			...
			# run model
			FLAGS["ensemble"]=2
			make_ensemble(school,fish,cons,fishtree,EVENTS,FLAGS,2);
...
function do_1fisher_1school_b()
    ...
    SIG = linspace(2,GRD_mx/10,10);
    FF  = linspace(2,GRD_mx/10,10);
...
function do_2fisher_1school()
    ...
    PRM.PS_n = int(round(.05/(pi*PRM.PF_sig^2/PRM.GRD_nx^2))); #force fish area
    ...
    JUMP   = logspace(log10(0.001),log10(.1),25); ## Tau_l
    CATCH  = logspace(log10(0.01),log10(1),25); ## Tau_h
    ...
    for i = 1:length(JUMP)
        for j = 1:length(CATCH)
            ...
            result_noinf[i,j] = PRM.PS_p,PRM.PC_q,mean(cons.measure["Ts"]),
            	mean(cons.measure["Hdist"]),PRM.PC_rp;
