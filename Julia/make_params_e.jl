...
global P_C_n,P_C_v,P_C_ff,P_C_c
...
const P_Cl_al  = 2.5;  # levy flight constant (ref 1)
...
##### Initialize clusters
Fclust_xy =rand(Float64,P_Cl_n,2).*repmat([Grd_nx*Grd_dx Grd_ny*Grd_dy],P_Cl_n,1);

##### Initial fish
Fish_xy = zeros(Float64,P_F_n,2); # location of fish
...
##### Initialize fishers
Cons_H     = zeros(Int,P_C_n); # catch at time t
Cons_xy    = zeros(Float64,P_C_n,2); # Fisher locations through time
Cons_xy[:,:,1] = rand(Float64,P_C_n,2).*repmat([Grd_nx*Grd_dx Grd_ny*Grd_dy],P_C_n,1);

##### Initialize waiting times
Tau_n     = zeros(Int,P_C_n); # number of events (hauls) so far
Tau_t     = zeros(Int,P_C_n); # time between current events (waiting time)
Tau_s     = zeros(Float64,P_C_n); # cumulative waiting time
Tau_mu    = zeros(Float64,P_C_n); # running average waiting time (wait_sum / wait_n)
Tau_dmu   = ones(Float64,P_C_n)*999; # min diff in mean waiting times between t and t-1
